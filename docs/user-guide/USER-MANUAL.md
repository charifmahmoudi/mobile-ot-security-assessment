...
The screenshot set below documents the preceding tested capture and inventory journeys from Android 15 emulator [CI run #20](https://github.com/charifmahmoudi/mobile-ot-security-assessment/actions/runs/33326876122). The current guided shell adds persistent **Overview**, **Collect**, **Assets**, **Findings** and **Report** destinations; its screenshot is generated by the new `guidedShellExposesFiveProfessionalAssessmentStages` instrumentation test.

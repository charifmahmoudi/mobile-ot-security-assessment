...
| Case App | Guided site workflow plus an encrypted Golden Customer Assessment workspace for preparation, dual approval, restart/resume and collection gating; passive import/review, inventory reasoning and findings/report-readiness UI; no Android `INTERNET` permission |
...
| Pilot case application path | The repo-owned Golden Customer Assessment can be created from an empty app state, edited before creation, prepared, moved to awaiting authorization, approved by named operational/security roles, restored after a host-forced process stop, moved through collection/review/finalization, and superseded into a successor revision. The workspace shows purpose, context, exact scope, exclusions, methods, policy, stop conditions, roles, approvals, reviewer decisions, lifecycle/revision and audit events | API 29/35 pilot instrumentation |
...
| Active operation | One Modbus/TCP Read Device Identification request, FC `0x2B` / MEI `0x0E`, basic objects only, to one authorized IPv4 target on TCP/502. The UI no longer self-authorizes with a confirmation checkbox: it restores a collecting professional aggregate, evaluates the domain operation guard and binds the broker grant to the recorded authorization-artifact hash | Codec, policy and end-to-end tests |
...
The Android safety workflow exercises builds, architecture checks, JVM tests, lint, API 29/35 instrumentation, passive capture imports, the Capture Broker file-descriptor journey, active Modbus behavior against PyModbus/modbus-tk/Conpot, and the native receive-only capture gate. On both Android API levels, the pilot harness launches a deterministic OT endpoint, creates and authorizes the Golden Customer Assessment from cleared application data, force-stops the Case App, verifies encrypted restoration before collection, then drives the same case through review, finalization and successor-revision preparation. Negative instrumentation covers missing approval, expired authority and stale scope binding. `core-domain` tests additionally exercise finalization/supersession and evidence-record invariants.
...
- generalized import of arbitrary professional-case preparation files and the normalized evidence/review schema; the current application path now covers editable pilot preparation, reviewer decisions and successor-revision flow, but it is not yet the completed evidence store;

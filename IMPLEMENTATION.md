...
| Pilot case application path | The repo-owned Golden Customer Assessment can be created from an empty app state, prepared, moved to awaiting authorization, approved by named operational/security roles, restored after a host-forced process stop, and moved into collection. The workspace shows purpose, context, exact scope, exclusions, methods, policy, stop conditions, roles, lifecycle/revision and audit events | API 29/35 pilot instrumentation |
...
The Android safety workflow exercises builds, architecture checks, JVM tests, lint, API 29/35 instrumentation, passive capture imports, the Capture Broker file-descriptor journey, active Modbus behavior against PyModbus/modbus-tk/Conpot, and the native receive-only capture gate. On both Android API levels, the pilot harness launches a deterministic OT endpoint, creates and authorizes the Golden Customer Assessment from cleared application data, force-stops the Case App, then verifies encrypted restoration before collection. Negative instrumentation covers missing approval, expired authority and stale scope binding. `core-domain` tests additionally exercise finalization/supersession and evidence-record invariants.
...
- general-purpose editing/import of arbitrary professional-case preparation fields, reviewer decisions and revision workflow; the current application path deliberately implements the deterministic pilot case through authorization/restart/collection rather than claiming the full lifecycle UI;
